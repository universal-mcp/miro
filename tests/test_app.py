...
@pytest.fixture
def app_instance():
    """Provides a MiroApp instance for tests."""
    mock_integration = MagicMock()
    mock_integration.get_credentials.return_value = {"access_token": "dummy_access_token"}

    return MiroApp(integration=mock_integration)

def test_universal_mcp_miro_app_initialization(app_instance):
    """
    Test that the MiroApp instance is initialized correctly with a name.
    """
    ...
    assert app_instance.name == "miro", "MiroApp instance has unexpected name."


def test_universal_mcp_miro_tool_docstrings_format(app_instance):
    """
    Test that each tool method in MiroApp has a well-formatted docstring,
...
def test_universal_mcp_miro_tools_are_callable(app_instance):
    """
    Test that each tool method returned by list_tools in MiroApp is callable.
    """
    tools = app_instance.list_tools()
    assert isinstance(tools, list), "list_tools() should return a list."
